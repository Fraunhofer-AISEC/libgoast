...
impl From<SyntaxNode> for RSAst {
    fn from(syntax: SyntaxNode) -> Self {
        ...
        println!(
            "Not correctly translating node of type: {}",
            kind.text().to_string()
        );
...
impl From<Const> for RSConst {
    fn from(node: Const) -> Self {
        RSConst {
            ...
            generic_params: node
                .generic_param_list()
                .map(|gpl| gpl.generic_params().into_iter().map(Into::into))
                .into_iter()
                .flatten()
                .collect_vec(),
        }
...
impl From<Enum> for RSEnum {
    fn from(node: Enum) -> Self {
        RSEnum {
            ...
            variants: node
                .variant_list()
                .map(|vl| vl.variants().into_iter().map(Into::into))
                .into_iter()
                .flatten()
                .collect_vec(),
            generic_params: node
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSExternBlock {
    pub(crate) ast_node: RSNode,
    extern_items: Vec<RSExternItem>,
    abi: Option<RSAbi>,
}
impl From<ExternBlock> for RSExternBlock {
    fn from(node: ExternBlock) -> Self {
        RSExternBlock {
            ...
            extern_items: node
                .extern_item_list()
                .map(|eil| eil.extern_items().map(Into::into).collect::<Vec<_>>())
                .unwrap_or_default(),
        }
    }
}
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSExternCrate {
    pub(crate) ast_node: RSNode,
...
impl From<ExternCrate> for RSExternCrate {
    fn from(node: ExternCrate) -> Self {
        RSExternCrate {
            ...
            rename: node
                .rename()
                .map(|rn| rn.name().map(|n| n.to_string()))
                .unwrap_or_default(),
        }
    }
}
...
impl From<Impl> for RSImpl {
    fn from(node: Impl) -> Self {
        RSImpl {
            ...
            items: node
                .assoc_item_list()
                .map(|ail| ail.assoc_items().map(Into::into).collect::<Vec<_>>())
                .unwrap_or_default(),
            path_types: node
                .syntax()
                .children()
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSMacroCall {
    pub(crate) ast_node: RSNode,
    path: Option<RSPath>,
    macro_string: String,
}
impl From<MacroCall> for RSMacroCall {
    fn from(node: MacroCall) -> Self {
        RSMacroCall {
            ast_node: node.syntax().into(),
...
impl From<Module> for RSModule {
    fn from(node: Module) -> Self {
        RSModule {
            ...
            items: node
                .item_list()
                .map(|il| il.items().map(Into::into).collect::<Vec<_>>())
                .unwrap_or_default(),
        }
    }
}
...
pub struct RSTypeAlias {
    ...
    name: Option<String>,
    ty: Option<RSType>,
    type_bound_list: Vec<RSTypeBound>,
    generic_params: Vec<RSGenericParam>,
}
impl From<TypeAlias> for RSTypeAlias {
    fn from(node: TypeAlias) -> Self {
        RSTypeAlias {
            ...
            name: node.name().map(|n| n.to_string()),
            ty: node.ty().map(Into::into),
            type_bound_list: node
                .type_bound_list()
                .iter()
                .flat_map(|tb| tb.bounds().map(Into::into))
                .collect::<Vec<_>>(),
            generic_params: node
                .generic_param_list()
                .map(|gpl| gpl.generic_params().into_iter().map(Into::into))
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSUseTree {
    pub(crate) ast_node: RSNode,
    path: Option<RSPath>,
    rename: Option<String>,
    use_trees: Vec<RSUseTree>,
    star: bool,
}
impl From<UseTree> for RSUseTree {
    fn from(node: UseTree) -> Self {
        RSUseTree {
            ...
            use_trees: node
                .use_tree_list()
                .map(|utl| utl.use_trees().map(Into::into).collect::<Vec<_>>())
                .unwrap_or_default(),
...
impl From<BlockExpr> for RSBlockExpr {
    fn from(node: BlockExpr) -> Self {
        let ret = RSBlockExpr {
            ...
            stmts: node
                .stmt_list()
                .map(|sl| sl.statements().map(Into::into).collect::<Vec<_>>())
                .unwrap_or_default(),
            tail_expr: node
                .stmt_list()
                .map(|sl| sl.tail_expr().map(Into::into))
                .flatten()
                .into_iter()
                .collect(),
        };
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSCallExpr {
    pub(crate) ast_node: RSNode,
    expr: Vec<RSExpr>,
    arguments: Vec<RSExpr>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSClosureExpr {
    pub(crate) ast_node: RSNode,
    param_list: Vec<RSParamList>,
    ret_type: Vec<RSType>,
    expressions: Vec<RSExpr>,
}
impl From<ClosureExpr> for RSClosureExpr {
    fn from(node: ClosureExpr) -> Self {
        RSClosureExpr {
            ...
            ret_type: node
                .ret_type()
                .and_then(|rt| rt.ty().map(Into::into))
                .into_iter()
                .collect::<Vec<_>>(),
            expressions: node
                .syntax()
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSLiteral {
    pub(crate) ast_node: RSNode,
    literal_type: RSLiteralType,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSLoopExpr {
    pub(crate) ast_node: RSNode,
    label: Option<String>,
    body: Vec<RSBlockExpr>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSMacroExpr {
    pub(crate) ast_node: RSNode,
    macro_call: Option<RSMacroCall>,
}
...
impl From<MatchExpr> for RSMatchExpr {
    fn from(node: MatchExpr) -> Self {
        RSMatchExpr {
            ...
            arms: node
                .match_arm_list()
                .map(|mal| mal.arms().map(Into::into).collect())
                .unwrap_or_default(),
        }
    }
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSPathExpr {
    pub(crate) ast_node: RSNode,
    pub path: Option<RSPath>,
}
    ...
    fn from(node: PathSegment) -> Self {
        RSPathSegment {
            ...
            type_args: node
                ...
                .map(|pal| {
                    pal.type_args()
                        .filter_map(|ta| ta.ty().map(Into::into))
                        .collect::<Vec<_>>()
                })
                .unwrap_or_default(),
            ret_type: node
                .ret_type()
                .map(|rty| rty.ty().map(Into::into))
                .flatten()
                .into_iter()
                .collect(),
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSTypeAnchor {
    pub(crate) ast_node: RSNode,
    path_ty: Vec<RSPathType>,
    ty: Vec<RSType>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSRecordExpr {
    pub(crate) ast_node: RSNode,
    path: Option<RSPath>,
    fields: Vec<RSRecordExprField>,
    spread: Vec<RSExpr>,
}
    ...
    fn from(node: RecordExpr) -> Self {
        RSRecordExpr {
            ...
            fields: node
                ...
                .map(|fl| fl.fields().map(Into::into).collect())
                .unwrap_or_default(),
            spread: node
                .record_expr_field_list()
                .map(|fl| fl.spread().map(Into::into))
                .unwrap_or_default()
                .into_iter()
                .collect(),
        }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSUnderscoreExpr {
    pub(crate) ast_node: RSNode,
}
impl From<UnderscoreExpr> for RSUnderscoreExpr {
    fn from(node: UnderscoreExpr) -> Self {
        RSUnderscoreExpr {
            ast_node: node.syntax().into(),
        }
    }
...
impl From<LetStmt> for RSLetStmt {
    fn from(node: LetStmt) -> Self {
        RSLetStmt {
            ...
            let_else: node
                .let_else()
                .map(|l| l.block_expr().map(Into::into))
                .flatten(),
            pat: node.pat().map(Into::into),
            ty: node.ty().map(Into::into),
        }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSLiteralPat {
    pub(crate) ast_node: RSNode,
    literal: Option<RSLiteral>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSRecordPat {
    pub(crate) ast_node: RSNode,
    path: Option<RSPath>,
    fields: Vec<RSRecordPatField>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSRestPat {
    pub(crate) ast_node: RSNode,
}
impl From<RestPat> for RSRestPat {
    fn from(node: RestPat) -> Self {
        RSRestPat {
            ast_node: node.syntax().into(),
        }
    }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSForType {
    pub(crate) ast_node: RSNode,
    ty: Vec<RSType>,
    generics_in_for: Vec<RSGenericParam>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSNeverType {
    pub(crate) ast_node: RSNode,
}
impl From<NeverType> for RSNeverType {
    fn from(node: NeverType) -> Self {
        RSNeverType {
            ast_node: node.syntax().into(),
        }
    }
...
pub struct RSPath {
    pub(crate) ast_node: RSNode,
    segment: Option<RSPathSegment>,
    qualifier: Vec<RSPath>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSNameRef {
    pub(crate) ast_node: RSNode,
    text: String,
    ident: Option<String>,
    int_number_token: Option<String>,
    has_cap_Self: bool,
    has_crate: bool,
    has_self: bool,
    has_super: bool,
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSTypeBound {
    pub(crate) ast_node: RSNode,
    ty: Option<RSType>,
    generics_in_for: Vec<RSGenericParam>,
    lifetime: Option<RSLifetime>,
    bound_generic_args: Vec<RSUseBoundGenericArg>,
} //use_bound_generic_arg: UseBoundGenericArg
    ...
    fn from(node: TypeBound) -> Self {
        RSTypeBound {
            ...
            generics_in_for: node
                ...
                .map(Into::into)
                .collect::<Vec<_>>(),
            bound_generic_args: node
                .use_bound_generic_args()
                .into_iter()
                .flat_map(|args| args.use_bound_generic_args().into_iter())
                .map(Into::into)
                .collect::<Vec<_>>(),
        }
    }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSRecordFieldList {
    pub(crate) ast_node: RSNode,
    fields: Vec<RSRecordField>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSTupleFieldList {
    pub(crate) ast_node: RSNode,
    fields: Vec<RSTupleField>,
}
    ...
    fn from(node: AssocTypeArg) -> Self {
        RSAssocTypeArg {
            ...
            generic_args: node
                .generic_arg_list()
                .iter()
                .flat_map(|gal| gal.generic_args().map(Into::into))
                .collect::<Vec<_>>(),
        }
    }
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSConstParam {
    pub(crate) ast_node: RSNode,
    default_val: Option<RSConstArg>,
    ty: Option<RSType>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSTypeParam {
    pub(crate) ast_node: RSNode,
    name: Option<String>,
    type_bound_list: Vec<RSTypeBound>,
    default_type: Option<RSType>,
}
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSAsmRegOperand {
    pub(crate) ast_node: RSNode,
}
impl From<AsmRegOperand> for RSAsmRegOperand {
    fn from(node: AsmRegOperand) -> Self {
        RSAsmRegOperand {
            ast_node: node.syntax().into(),
        }
    }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSAsmClobberAbi {
    pub(crate) ast_node: RSNode,
}
impl From<AsmClobberAbi> for RSAsmClobberAbi {
    fn from(node: AsmClobberAbi) -> Self {
        RSAsmClobberAbi {
            ast_node: node.syntax().into(),
        }
    }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSAsmOperandNamed {
    pub(crate) ast_node: RSNode,
}
impl From<AsmOperandNamed> for RSAsmOperandNamed {
    fn from(node: AsmOperandNamed) -> Self {
        RSAsmOperandNamed {
            ast_node: node.syntax().into(),
        }
    }
...
#[derive(uniffi::Record, Debug, Clone, PartialEq, Eq, Hash)]
pub struct RSAsmOptions {
    pub(crate) ast_node: RSNode,
}
impl From<AsmOptions> for RSAsmOptions {
    fn from(node: AsmOptions) -> Self {
        RSAsmOptions {
            ast_node: node.syntax().into(),
        }
    }
